...
TYPE, public :: oft_gs_xmhd_sim
  ...
  TYPE(oft_lusolver), POINTER :: pre => NULL() !< Preconditioner using vacuum operator
...
CALL build_approx_jacobian(self,self%nlfun%jac_op)
self%jacobian_block_mask(1,1) = 3
...
!CALL self%vac_pre%apply(self%u, self%tmp)
!write(*,*) 'hi'
...
real(r8), pointer :: plot_vals(:), tmp_arr(:)
...
CALL self%tmp%get_local(plot_vals,2)
...
REAL(r8) :: p_source, f_source, diag_vals(2), chi
REAL(r8) ::  psi, dpsi(3), diag(2), coords(3), jac_det, jac_mat(3,4), T, dT(3)
REAL(r8), ALLOCATABLE, DIMENSION(:) :: basis_vals, psi_weights_loc, T_weights_loc
REAL(r8), ALLOCATABLE, DIMENSION(:,:) :: basis_grads,res_loc
REAL(r8), POINTER, DIMENSION(:) :: psi_weights, psi_res, alam_vals, T_weights, T_res
quad=>oft_blagrange%quad
NULLIFY( psi_weights, psi_res, T_weights, T_res,alam_vals)
...
!$omp jac_det,psi,dpsi, T, dT) reduction(+:diag_vals)
!Edit for new fields
  ...
  DO m=1,quad%np
    ...
    DO jr=1,oft_blagrange%nce
      res_loc(jr,2) = res_loc(jr,2) &
        + basis_vals(jr)*T*jac_det*quad%wts(m)*coords(1) &
...
DEALLOCATE(basis_vals,basis_grads, psi_weights_loc, T_weights_loc,cell_dofs,res_loc)
!$omp end parallel
! Apply BCs
...
CALL b%restore_local(psi_res,1,add=.TRUE., wait = .TRUE.)
...
CALL b%get_local(psi_res, 1)
CALL ptmp%delete
DEALLOCATE(psi_res,T_res, ptmp,alam_vals)
...
INTEGER(i4) :: i,m,jr, k,l
INTEGER(i4), ALLOCATABLE, DIMENSION(:) :: cell_dofs
REAL(r8) :: eta_loc, curr_loc, chi
REAL(r8) ::  psi, dpsi(3), coords(3), jac_det, jac_mat(3,4), T, dT(3)
REAL(r8), ALLOCATABLE, DIMENSION(:) :: basis_vals, psi_weights_loc, T_weights_loc
REAL(r8), ALLOCATABLE, DIMENSION(:,:) :: basis_grads, res_loc
REAL(r8), POINTER, DIMENSION(:) :: psi_weights, psi_res, alam_vals, T_weights, T_res
quad=>self%eq%fe_rep%quad
NULLIFY( psi_weights, T_weights,psi_res, T_res,alam_vals)
!---Get weights from solution vector
CALL a%get_local(psi_weights,1)
CALL a%get_local(T_weights,2)
CALL self%eq%psi%restore_local(psi_weights)
...
CALL b%set(0.d0)
CALL b%get_local(psi_res,1)
CALL b%get_local(T_res,1)
!$omp parallel private(m,jr,curved,coords,cell_dofs,basis_vals,basis_grads, &
!$omp psi_weights_loc,T_weights_loc,res_loc,jac_mat, &
!$omp jac_det,psi,dpsi,T,dT, &
...
class (oft_matrix), pointer, intent(inout) :: mat
REAL(r8), ALLOCATABLE, DIMENSION(:) :: basis_vals, psi_weights_loc, T_weights_loc
REAL (r8) :: coords(3), eta_loc, jac_det, jac_mat(3,4), chi
REAL(r8), ALLOCATABLE, DIMENSION(:,:) :: basis_grads
type(oft_local_mat), allocatable, dimension(:,:) :: jac_loc
CLASS(oft_vector), POINTER :: oft_lag_vec
INTEGER(i4), ALLOCATABLE, DIMENSION(:), TARGET :: cell_dofs
  ...
  DO m=1,quad%np
    ...
    DO jr=1,oft_blagrange%nce
      DO jc=1,oft_blagrange%nce
        ...
        + self%dt * chi * DOT_PRODUCT(basis_grads(:, jc),basis_grads(:, jr))*jac_det*quad%wts(m)*coords(1)
      END DO
    END DO
  END DO
  !---Get local to global DOF mapping
...
!---Apply bc to local matrix
  DO jr=1,oft_blagrange%nce
    IF(oft_blagrange%be(cell_dofs(jr))) jac_loc(1,1)%m(jr,:)=0.d0
  END DO
  CALL self%fe_rep%mat_zero_local_rows(jac_loc,self%T_bc(cell_dofs),2)
  CALL self%fe_rep%mat_add_local(mat,jac_loc,iloc,tlocks)
END DO
deallocate(cell_dofs,basis_vals,basis_grads,jac_loc)
!$omp end parallel
...
! apply free boundary BCs to psi
CALL set_bcmat_mod(self%eq,mat, 1,1)
CALL fem_dirichlet_diag(oft_blagrange,mat,self%T_bc,2)
...
SUBROUTINE graph_deep_copy(src, dest)
  ...
  dest%ncg = src%ncg
  dest%nnz = src%nnz
  ALLOCATE(dest%kr(src%nnz))
  ALLOCATE(dest%lc(src%nnz))
  dest%kr = src%kr
  dest%lc = src%lc
END SUBROUTINE
