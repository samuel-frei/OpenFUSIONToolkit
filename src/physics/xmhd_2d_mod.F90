...
USE oft_gs, ONLY: gs_epsilon, build_dels_mug
IMPLICIT NONE
#include "local.h"
...
TYPE, extends(oft_noop_matrix) :: xmhd_2d_nlfun
  ...
  PROCEDURE :: nlfun_rhs_apply => nlfun_rhs_apply
...
type(oft_lag_bginterp) :: grad_psi, grad_by
...
call oft_blagrange%vec_create(grad_by%u)
...
CALL grad_by%u%restore_local(plot_vals)
CALL grad_by%setup(oft_blagrange)
CALL oft_blag_vproject(oft_blagrange,grad_by,ux,uy,uz)
CALL v_lag%set(0.d0)
CALL lminv%apply(v_lag,ux)
...
DO i=1,self%nsteps
  ...
  CALL self%nlfun%nlfun_rhs_apply(u,v)
...
LOGICAL :: curved, cyl_flag, vac
INTEGER(i4) :: i,m,jr, k,l
INTEGER(i4), ALLOCATABLE, DIMENSION(:) :: cell_dofs
REAL(r8) :: k_boltz = elec_charge
REAL(r8) :: m_i=proton_mass
REAL(r8) :: T0= 4.6E3
REAL(r8) :: chi,  nu, D_diff, gamma, diag_vals(7), B_0(3), eta_loc, eta_loc_inv
REAL(r8) :: n, vel(3), T, psi, by, dT(3),dn(3),dpsi(3),dby(3),&
         dvel(3,3),div_vel,jac_mat(3,4), jac_det,btmp(3), tmp1(3), coords(3)
...
!$omp jac_det,T,n,psi,by,vel,dT,dn,dpsi,dby,dvel,div_vel,btmp, tmp1, &
!$omp eta_loc,eta_loc_inv, vac) reduction(+:diag_vals)
!Edit for new fields
...
DO i=1,mesh%nc
  ...
  vac = .FALSE.
  IF (ASSOCIATED(self%region_flag)) THEN
    IF (self%region_flag(mesh%reg(i)) == 2) THEN
      vac = .TRUE. !If in a vacuum region
    END IF
  END IF
  !---------------------------------------------------------------------------
  ! Quadrature Loop
  ...
  DO m=1,quad%np
    ...
    ! Note: in cylindrical mode, by = F = rB_phi
    ...
    ! set temperature dependent resistivity
    IF (ASSOCIATED(self%region_flag)) THEN
      IF (self%region_flag(mesh%reg(i)) == 1) THEN
        eta_loc = eta(mesh%reg(i))*(T0/T)**1.5
      END IF
    END IF   

    eta_loc_inv = 1.0/eta_loc 
    ...
    DO jr=1,oft_blagrange%nce
      ...
      IF (cyl_flag) THEN
        IF(vac) THEN
            ...
            + DOT_PRODUCT(basis_grads(:,jr), dpsi)*jac_det*quad%wts(m)/(coords(1)+gs_epsilon)
        ELSE
            res_loc(jr,6) = res_loc(jr,6) &
            + eta_loc_inv*mu0*basis_vals(jr)*psi*jac_det*quad%wts(m)/(self%dt*(coords(1)+gs_epsilon)) & 
            + eta_loc_inv*mu0*basis_vals(jr)*DOT_PRODUCT(vel, dpsi)*jac_det*quad%wts(m)/(coords(1)+gs_epsilon) &
            + eta_loc_inv*mu0*basis_vals(jr)*tmp1(2)*jac_det*quad%wts(m)/(coords(1)+gs_epsilon) &
            + DOT_PRODUCT(basis_grads(:,jr), dpsi)*jac_det*quad%wts(m)/(coords(1)+gs_epsilon)
        END IF
        ! IF(vac) THEN
        !     res_loc(jr,6) = res_loc(jr,6) &
        !     + eta_loc*self%dt*DOT_PRODUCT(basis_grads(:,jr), dpsi)*jac_det*quad%wts(m)/(mu0*(coords(1)+gs_epsilon))
        ! ELSE
        !     res_loc(jr,6) = res_loc(jr,6) &
        !     + basis_vals(jr)*psi*jac_det*quad%wts(m)/((coords(1)+gs_epsilon)) & 
        !     + self%dt*basis_vals(jr)*DOT_PRODUCT(vel, dpsi)*jac_det*quad%wts(m)/(coords(1)+gs_epsilon) &
        !     + self%dt*basis_vals(jr)*tmp1(2)*jac_det*quad%wts(m)/(coords(1)+gs_epsilon) &
        !     + eta_loc*self%dt*DOT_PRODUCT(basis_grads(:,jr), dpsi)*jac_det*quad%wts(m)/(mu0*(coords(1)+gs_epsilon)) &
        !     - self%dt*basis_vals(jr)*1.5*eta_loc*DOT_PRODUCT(dpsi, dT)*jac_det*quad%wts(m)/(mu0*T*(coords(1)+gs_epsilon))
        ! END IF
      ELSE
        IF(vac) THEN
          res_loc(jr, 6) = res_loc(jr, 6) &
          + DOT_PRODUCT(basis_grads(:,jr), dpsi)*jac_det*quad%wts(m)
        ELSE
          res_loc(jr, 6) = res_loc(jr, 6) &
          + mu0*basis_vals(jr)*psi*jac_det*quad%wts(m)/(eta_loc*self%dt) &
          + mu0*basis_vals(jr)*DOT_PRODUCT(vel, dpsi)*jac_det*quad%wts(m)/eta_loc &
          + mu0*basis_vals(jr)*tmp1(2)*jac_det*quad%wts(m)/eta_loc &
          + DOT_PRODUCT(basis_grads(:,jr), dpsi)*jac_det*quad%wts(m)
        END IF
      END IF
      ...
      ELSE
        ...
        + basis_vals(jr)*self%dt*DOT_PRODUCT(vel, dby)*jac_det*quad%wts(m) &
        + basis_vals(jr)*self%dt*by*div_vel*jac_det*quad%wts(m) &
        + self%dt*eta_loc*DOT_PRODUCT(basis_grads(:,jr), dby)*jac_det*quad%wts(m)/mu0
      END IF 
    END DO
  END DO
...
!---------------------------------------------------------------------------
!> Compute the RHS of the NL error function
!---------------------------------------------------------------------------
subroutine nlfun_rhs_apply(self,a,b)
class(xmhd_2d_nlfun), intent(inout) :: self !< NL function object
class(oft_vector), target, intent(inout) :: a !< Source field
class(oft_vector), intent(inout) :: b !< Result of metric function
type(oft_quad_type), pointer :: quad
...
DEALLOCATE(n_res,velx_res,vely_res, velz_res, T_res, psi_res, by_res, &
        n_weights,vel_weights, T_weights, psi_weights, by_weights)
end subroutine nlfun_rhs_apply
!---------------------------------------------------------------------------
!> Needs docs
!---------------------------------------------------------------------------
...
REAL(r8) :: T0 = 4.6E3
REAL(r8) :: m_i = proton_mass
REAL(r8) :: chi, nu, D_diff, gamma, B_0(3), diag_vals(7), eta_loc, eta_loc_inv
REAL(r8) :: n, vel(3), T, psi, by, dT(3),dn(3),dpsi(3),dby(3),&
dvel(3,3),div_vel,jac_mat(3,4), jac_det,btmp(3), tmp1(3),tmp2(3), tmp3(3), coords(3)
REAL (r8) :: eta(mesh%nreg)
...
!$omp tmp1, tmp2, tmp3, eta_loc,eta_loc_inv, vac) 
ALLOCATE(basis_vals(oft_blagrange%nce),basis_grads(3,oft_blagrange%nce))
...
DO i=1,mesh%nc
  ...
  IF (ALLOCATED(self%region_flag)) THEN
    IF (self%region_flag(mesh%reg(i)) == 2) vac = .TRUE. !If in a vacuum region
  END IF
!---------------------------------------------------------------------------
! Quadrature Loop
!---------------------------------------------------------------------------
      ...
      DO jc=1,oft_blagrange%nce
        ...
        ELSE
          DO l=1,3
            ...
            - self%dt*nu*basis_vals(jc)*DOT_PRODUCT(basis_grads(:,jr), dvel(l, :))*jac_det*quad%wts(m)/(m_i*n**2) & !-- not sure if indexing on dvel is right here
            - self%dt*nu*basis_vals(jr)*DOT_PRODUCT(basis_grads(:,jc), dvel(l, :))*jac_det*quad%wts(m)/(m_i*n**2) &
            + self%dt*nu*basis_vals(jr)*2.d0*basis_vals(jc)*DOT_PRODUCT(dn, dvel(l, :))*jac_det*quad%wts(m)/(m_i*n**3)
          END DO
        END IF
        !--vel, vel
        IF (cyl_flag) THEN
        ...
        ELSE
          DO k=1,3
            jac_loc(k+1,k+1)%m(jr,jc)= jac_loc(k+1,k+1)%m(jr,jc) &
              ...
              + self%dt*nu*DOT_PRODUCT(basis_grads(:,jr), basis_grads(:,jc))*jac_det*quad%wts(m)/(m_i*n) &
              - self%dt*nu*basis_vals(jr)*DOT_PRODUCT(dn, basis_grads(:,jc))*jac_det*quad%wts(m)/(m_i*n**2)
            DO l=1,3
              jac_loc(k+1,l+1)%m(jr,jc)= jac_loc(k+1,l+1)%m(jr,jc) &
        ...
        ELSE
          ...
          DO l=1,3
            ...
            - self%dt*basis_vals(jr)*DOT_PRODUCT(dn, tmp2)*btmp(l)*jac_det*quad%wts(m)/(m_i*n**2*mu0) &
            - self%dt*basis_vals(jr)*DOT_PRODUCT(dn, btmp)*tmp2(l)*jac_det*quad%wts(m)/(m_i*n**2*mu0) &
            + self%dt*basis_vals(jr)*dn(l)*DOT_PRODUCT(tmp2, btmp)*jac_det*quad%wts(m)/(m_i*n**2*mu0) 
          END DO
        END IF
        ! --vel, by
        IF (cyl_flag) THEN
        ...
        ELSE
        jac_loc(5, 1)%m(jr,jc) = jac_loc(5, 1)%m(jr, jc) &  
        - self%dt*chi*basis_vals(jr)*DOT_PRODUCT(basis_grads(:, jc), dT)*jac_det*quad%wts(m)/n & ! grad(delta_n) 
        + self%dt*chi*basis_vals(jr)*basis_vals(jc)*DOT_PRODUCT(dn, dT)*jac_det*quad%wts(m)/(n**2) ! delta_n 
        END IF
        ! T, vel
        IF (cyl_flag) THEN
          DO l=1,3
        ...
        ELSE
          ...
          - self%dt*basis_vals(jr)*chi*DOT_PRODUCT(dn, basis_grads(:, jc))*jac_det*quad%wts(m)/n 
        END IF
        ! Induction
        ! psi, psi
        IF (cyl_flag) THEN
          IF (vac) THEN
            ...
            + DOT_PRODUCT(basis_grads(:,jr),basis_grads(:,jc))*jac_det*quad%wts(m)/(coords(1)+gs_epsilon)
          ELSE
            jac_loc(6, 6)%m(jr,jc) = jac_loc(6, 6)%m(jr,jc) &
            + eta_loc_inv*mu0*basis_vals(jr)*basis_vals(jc)*jac_det*quad%wts(m)/(self%dt*(coords(1)+gs_epsilon)) &
            + eta_loc_inv*mu0*basis_vals(jr)*DOT_PRODUCT(vel,basis_grads(:,jc))*jac_det*quad%wts(m)/(coords(1)+gs_epsilon) &
            + DOT_PRODUCT(basis_grads(:,jr),basis_grads(:,jc))*jac_det*quad%wts(m)/(coords(1)+gs_epsilon)
          END IF
          ! IF (vac) THEN
          !   jac_loc(6, 6)%m(jr,jc) = jac_loc(6, 6)%m(jr,jc) &
          ...
          !   + self%dt*basis_vals(jr)*DOT_PRODUCT(vel,basis_grads(:,jc))*jac_det*quad%wts(m)/(coords(1)+gs_epsilon) &
          !   + self%dt*eta_loc*DOT_PRODUCT(basis_grads(:,jr),basis_grads(:,jc))*jac_det*quad%wts(m)/(mu0*(coords(1)+gs_epsilon)) &
          !   - self%dt*basis_vals(jr)*1.5*eta_loc*DOT_PRODUCT(basis_grads(:,jc), dT)*jac_det*quad%wts(m)/(mu0*T*(coords(1)+gs_epsilon))
          ! END IF
        ELSE
          IF (vac) THEN
            jac_loc(6, 6)%m(jr,jc) = jac_loc(6, 6)%m(jr,jc) &
             + DOT_PRODUCT(basis_grads(:,jr),basis_grads(:,jc))*jac_det*quad%wts(m)
          ELSE
            ! --psi, psi
            jac_loc(6, 6)%m(jr,jc) = jac_loc(6, 6)%m(jr,jc) &
            + mu0*basis_vals(jr)*basis_vals(jc)*jac_det*quad%wts(m)/(eta_loc*self%dt) &
            + mu0*basis_vals(jr)*DOT_PRODUCT(vel,basis_grads(:,jc))*jac_det*quad%wts(m)/eta_loc &
            + DOT_PRODUCT(basis_grads(:,jr),basis_grads(:,jc))*jac_det*quad%wts(m)
          END IF
        END IF
        !psi, v
        IF (cyl_flag) THEN
          DO l=1,3
            ...
            + eta_loc_inv*mu0*basis_vals(jr)*basis_vals(jc)*dpsi(l)*jac_det*quad%wts(m)/(coords(1)+gs_epsilon)
            tmp2(l) = 1.d0
            tmp3 = cross_product(B_0, tmp2)
            jac_loc(6,l+1)%m(jr,jc) = jac_loc(6,l+1)%m(jr,jc) &
            + eta_loc_inv*mu0*basis_vals(jr)*basis_vals(jc)*tmp3(2)*jac_det*quad%wts(m)/(coords(1)+gs_epsilon)
          END DO
          ! DO l=1,3
          !   tmp2 = [0.d0, 0.d0,0.d0]
          !   jac_loc(6,l+1)%m(jr,jc) = jac_loc(6,l+1)%m(jr,jc) &
          !   + self%dt*basis_vals(jr)*basis_vals(jc)*dpsi(l)*jac_det*quad%wts(m)/(coords(1)+gs_epsilon)
          !   tmp2(l) = 1.d0
          !   tmp3 = cross_product(B_0, tmp2)
          !   jac_loc(6,l+1)%m(jr,jc) = jac_loc(6,l+1)%m(jr,jc) &
          !   + self%dt*basis_vals(jr)*basis_vals(jc)*tmp3(2)*jac_det*quad%wts(m)/(coords(1)+gs_epsilon)
          ! END DO
        ELSE
          DO l=1,3
            tmp2 = [0.d0, 0.d0,0.d0]
            jac_loc(6,l+1)%m(jr,jc) = jac_loc(6,l+1)%m(jr,jc) &
            + mu0*basis_vals(jr)*basis_vals(jc)*dpsi(l)*jac_det*quad%wts(m)/eta_loc
            tmp2(l) = 1.d0
            tmp3 = cross_product(B_0, tmp2)
            jac_loc(6,l+1)%m(jr,jc) = jac_loc(6,l+1)%m(jr,jc) &
            + mu0*basis_vals(jr)*basis_vals(jc)*tmp3(2)*jac_det*quad%wts(m)/eta_loc
          END DO
        END IF
        !psi, T
        tmp1 = cross_product(dpsi,dvel(2, :))
        IF (ALLOCATED(self%region_flag)) THEN
          IF (self%region_flag(mesh%reg(i)) == 1) THEN
              jac_loc(6,5)%m(jr,jc) = jac_loc(6,5)%m(jr,jc) &
              + (mu0*basis_vals(jr)*psi*jac_det*quad%wts(m)/(self%dt*(coords(1)+gs_epsilon)) & 
              + mu0*basis_vals(jr)*DOT_PRODUCT(vel, dpsi)*jac_det*quad%wts(m)/(coords(1)+gs_epsilon) &
              + mu0*basis_vals(jr)*tmp1(2)*jac_det*quad%wts(m)/(coords(1)+gs_epsilon))*1.5*eta_loc_inv*basis_vals(jc)/T
          END IF
        END IF 
        ! IF (ALLOCATED(self%region_flag)) THEN
        !   IF (self%region_flag(mesh%reg(i)) == 1) THEN
        !       jac_loc(6,5)%m(jr,jc) = jac_loc(6,5)%m(jr,jc) &
        !       - 1.5*eta_loc*self%dt*basis_vals(jc)*DOT_PRODUCT(basis_grads(:,jr), dpsi)*jac_det*quad%wts(m)/(mu0*T*(coords(1)+gs_epsilon)) &
        !       + 1.5*2.5*eta_loc*self%dt*basis_vals(jr)*basis_vals(jc)*DOT_PRODUCT(dpsi, dT)*jac_det*quad%wts(m)/(mu0*T**2*(coords(1)+gs_epsilon)) &
        !       - 1.5*eta_loc*self%dt*basis_vals(jr)*DOT_PRODUCT(dpsi, basis_grads(:,jc))*jac_det*quad%wts(m)/(mu0*T*(coords(1)+gs_epsilon))
        !   END IF
        ! END IF 
        !--by, vel
        ...
        ELSE
          ...
          END DO
          jac_loc(7,3)%m(jr,jc) = jac_loc(7, 3)%m(jr,jc) &
          - basis_vals(jr)*self%dt*tmp2(2)*jac_det*quad%wts(m)
        END IF
        !-- by, psi
        ...
        ELSE
          ...
          + self%dt*eta_loc*DOT_PRODUCT(basis_grads(:,jr),basis_grads(:,jc))*jac_det*quad%wts(m)/mu0 
        END IF
      END DO
    END DO
  END DO
...
IF (ALLOCATED(self%region_flag)) THEN
  ...
  WHERE (self%psi_bc .OR. boundary_flag)
    self%psi_bc = .TRUE.
  END WHERE
  WHERE (self%by_bc .OR. boundary_flag)
    self%by_bc = .TRUE.
  END WHERE
END IF
...
! !--- Blocks to disable field evolutions in certain regions
! ALLOCATE(cell_dofs(oft_blagrange%nce))
...
! coil_ind = 7
! DO i=1,mesh%nc
!   IF(mesh%reg(i) == 1) THEN ! if we are in a plasma region 
!     call oft_blagrange%ncdofs(i,cell_dofs) ! Get global index of local DOFs
!     DO j=1, SIZE(cell_dofs)
!       self%n_bc(cell_dofs(j)) = .FALSE. ! allow density evolution in plasma
!       self%velx_bc(cell_dofs(j)) = .FALSE. ! allow velocity evolution in plasma
!       self%vely_bc(cell_dofs(j)) = .FALSE. ! allow velocity evolution in plasma
!       self%velz_bc(cell_dofs(j)) = .FALSE. ! allow velocity evolution in plasma
!       self%T_bc(cell_dofs(j)) = .FALSE. ! allow temperature evolution in plasma
!     END DO
!   ELSE IF(mesh%reg(i) > coil_ind) THEN !if we are in a coil region
!     call oft_blagrange%ncdofs(i,cell_dofs)
!     DO j=1, SIZE(cell_dofs)
!       self%psi_bc(cell_dofs(j)) = .TRUE. !block psi evolution in coils
!       self%by_bc(cell_dofs(j)) = .TRUE.  ! block F evolution in coils
!     END DO
!   END IF
! END DO
...
! write(*,*) 'oft_blagrange%ne:',oft_blagrange%ne
! write(*,*) 'mesh%np:',mesh%np
! write(*,*) 'mesh%ne:',mesh%ne
! write(*,*) 'mesh%reg:',SIZE(mesh%reg)
!ALLOCATE(self%n_bc(oft_blagrange%ne)); self%n_bc=.TRUE.
!ALLOCATE(self%velx_bc(oft_blagrange%ne)); self%velx_bc=.TRUE.
!ALLOCATE(self%vely_bc(oft_blagrange%ne)); self%vely_bc=.TRUE.
!ALLOCATE(self%velz_bc(oft_blagrange%ne)); self%velz_bc=.TRUE.
