...
ALLOCATE(n_eq(npoints))
ALLOCATE(v_init(npoints))
CALL hdf5_read(p_eq,TRIM(filename),"tokamaker/P",success)

CALL hdf5_field_get_sizes(TRIM(filename),"region_info/ETA",ndims,dim_sizes)
npoints = dim_sizes(1)
...
CALL mesh%save_vertex_scalar(n_eq,mhd_sim%xdmf_plot,'n0')
mhd_sim%den_scale = den_scale
n_eq = n_eq / den_scale
CALL mhd_sim%u%restore_local(n_eq,1)
...
! CALL mesh%save_vertex_scalar(v_init,mhd_sim%xdmf_plot,'vz0')
! CALL mhd_sim%u%restore_local(v_init,4)
field_init%func=>const_init
CALL oft_blag_project(ML_oft_blagrange%current_level,field_init,v)
CALL u%set(0.d0)
CALL minv%apply(u,v)
...
!---------------------------------------------------------------------------
mhd_sim%cyl_flag = .TRUE. ! Set solver mode to cylindrical
mhd_sim%chi=chi
mhd_sim%nu=nu
mhd_sim%gamma=gamma
