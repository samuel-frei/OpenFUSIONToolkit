...
! !! NEW METHOD !!
! ! !Compute necessary quantities for toroidal flux quench
! vac_int = 0.d0
...
! gs_td%tflux_source = dia_flux*0.1
! gs_td%evolve_F = .TRUE.
! CALL gs_td%setup(mg_mesh, mg_mesh_1)

! NULLIFY(tmp_arr)
! CALL gs_td%u%get_local(tmp_arr,6) !set psi to 0 to start
! tmp_arr = 0.d0*tmp_arr
! CALL gs_td%u%restore_local(tmp_arr,6) !set psi to 0 to start
...
gs_td%dt = 0.001d0 
gs_td%eq%itor_target = 0.9*gs_td%eq%itor_target
CALL gs_td%add_timestep(gs_td%dt)

CALL gs_td%u%get_local(tmp_arr,6) 
CALL equil%psi%restore_local(tmp_arr)
CALL gs_update_bounds(equil, track_opoint = .TRUE.)
dia_flux = 0.d0
CALL gs_comp_globals(equil, dummy_1, dummy_2, dummy_3, dummy_4, dia_flux, dummy_5, dummy_6)
write(*,*) dia_flux
...
! tmp_arr =0.2d0*psi_plasma
! CALL gs_td%u%restore_local(tmp_arr,6)
! CALL gs_td%add_timestep(gs_td%dt)
! CALL gs_td%u%get_local(tmp_arr,6)
! tmp_arr = tmp_arr + 0.2d0*psi_plasma
! CALL gs_td%u%restore_local(tmp_arr,6)
! CALL gs_td%add_timestep(gs_td%dt)
! CALL gs_td%u%get_local(tmp_arr,6)
! tmp_arr = tmp_arr + 0.2d0*psi_plasma
...
! CALL gs_td%add_timestep(gs_td%dt)
! CALL gs_td%u%restore_local(tmp_arr,6)
! psi_total = psi_plasma*0.1d0
! CALL gs_td%add_timestep(gs_td%dt)
! CALL gs_td%add_timestep(gs_td%dt)
! CALL gs_td%add_timestep(gs_td%dt)
! CALL gs_td%add_timestep(gs_td%dt)

! !Run simulation -> TQ followed by CQ
...
! !------------------------------------------------------
! gs_td%dt = dt_CQ/200.d0
! ! gs_td%pm = .TRUE.
! DO j=1, 200
!   write(*,*) j
!   gs_td%eq%itor_target = 7.87E6*(1.d0-.005d0*j)*mu0
...
! ! gs_td%dt = dt_CQ
! !  CALL gs_td%add_timestep(gs_td%dt)
! ! ! gs_td%eq%ip_ratio_target = 100.d0
! ! ! CALL gs_td%add_timestep(gs_td%dt)

! ! DO j=1, 10
! !   gs_td%eq%ip_ratio_target = 0.01d0/(1.d0-0.0999d0*j)
! !   write(*,*) gs_td%eq%ip_ratio_target 
! !   CALL gs_td%add_timestep(gs_td%dt)
! ! END DO
